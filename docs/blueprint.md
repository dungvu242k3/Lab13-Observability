...
## 1. Team Metadata
- **GROUP_NAME**: C401-C1
- **REPO_URL**: [https://github.com/dungvu242k3/Lab13-Observability](https://github.com/dungvu242k3/Lab13-Observability)
- **MEMBERS**:
  - Member A: Vũ Việt Dũng (2A202600444) | Role: Logging & PII
  - Member B: Vũ Tiến Thành (2A202600443) | Role: Tracing & Enrichment
  - Member C: Phan Thị Mai Phương (2A202600281) | Role: SLO & Alerts
  - Member D: Phạm Minh Trí (2A202600264) | Role: Load Test & Dashboard
  - Member E: Nguyễn Mậu Lân (2A202600400) | Role: Demo & Report
...
### 3.2 Dashboard & SLOs
- **DASHBOARD_6_PANELS_SCREENSHOT**: ![alt text](dashboard.png)
- **SLO_TABLE**:
...
![alt text](alerts_config.png)
- **SAMPLE_RUNBOOK_LINK**: [alerts.md](alerts.md)
...
- **TASKS_COMPLETED**: Xây dựng custom logic trong [pii.py](../app/pii.py). Viết hệ thống Regular Expression (Regex) để truy quét và che đậy (Redact) triệt để các dữ liệu y tế nhạy cảm (Mã thẻ BHYT, Tuổi bệnh nhân, Mã hồ sơ). Cấu hình Structured Logging tại [logging_config.py](../app/logging_config.py).
- **EVIDENCE_LINK**: [pii.py](../app/pii.py), [logging_config.py](../app/logging_config.py)

### Vũ Tiến Thành - Kỹ sư Hệ Thống (Tracing & Enrichment)
- **TASKS_COMPLETED**: Thiết lập `CorrelationIdMiddleware` tại [middleware.py](../app/middleware.py) để ép mỗi phiên hỏi/đáp có một `x-request-id` xuyên suốt. Gắn decorator `@observe` tại [tracing.py](../app/tracing.py) vào các hàm RAG và LLM để đo đạc Waterfall latency.
- **EVIDENCE_LINK**: [middleware.py](../app/middleware.py), [tracing.py](../app/tracing.py)

### Phan Thị Mai Phương - Kỹ sư Đảm bảo Chất lượng (Core & Alerts)
- **TASKS_COMPLETED**: Ráp nối Context Enrichment tại [main.py](../app/main.py) giúp gán thông tin user/session vào mọi bản ghi log. Trực tiếp soạn thảo cẩm nang Runbook hỗ trợ xử lý sự cố tại [alerts.md](alerts.md).
- **EVIDENCE_LINK**: [main.py](../app/main.py), [alerts.md](alerts.md)

### Phạm Minh Trí - Frontend & Performance (Data & Dashboard)
- **TASKS_COMPLETED**: Xây dựng giao diện "MedBot Health Dashboard" tại [dashboard.html](../dashboard.html). Làm giàu bộ dữ liệu truy vấn mẫu tại [sample_queries.jsonl](../data/sample_queries.jsonl) để thử nghiệm các kịch bản load test phức tạp.
- **EVIDENCE_LINK**: [dashboard.html](../dashboard.html), [sample_queries.jsonl](../data/sample_queries.jsonl) 

### Nguyễn Mậu Lân - Trưởng Nhóm / QA (Demo & Logic)
- **TASKS_COMPLETED**: Triển khai logic Semantic Caching tại [mock_llm.py](../app/mock_llm.py) và đo đạc hiệu năng RAG tại [mock_rag.py](../app/mock_rag.py). Tổng hợp các bằng chứng kỹ thuật và hoàn thiện báo cáo [blueprint-template.md](blueprint-template.md).
- **EVIDENCE_LINK**: [mock_llm.py](../app/mock_llm.py),  [mock_rag.py](../app/mock_rag.py), [blueprint-template.md](blueprint-template.md)
